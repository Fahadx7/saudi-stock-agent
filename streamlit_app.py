...
st.markdown("""
    ...
    /* إشعار جانبي عائم (Floating Side Notification) */
    .floating-notification {
        ...
        align-items: center;
        gap: 12px;
        animation: slideInLeft 0.5s ease-out, fadeOut 0.5s ease-in 295s forwards;
        max-width: 350px;
        direction: rtl;
    }
...
def get_stock_data(ticker, period="1y"):
    try:
        ...
        data = yf.download(ticker, period=period, interval="1d", auto_adjust=True, progress=False, timeout=20)
        
        if data is None or data.empty:
            return None, None

        # معالجة بيانات yfinance لضمان الحصول على أعمدة مسطحة (Flat Columns)
        if isinstance(data.columns, pd.MultiIndex):
            # إذا كان الرمز موجوداً في المستوى الأول (حدث مع النسخ الجديدة)
            if ticker in data.columns.get_level_values(0):
                data = data[ticker]
            else:
                # محاولة دمج المستويات أو أخذ المستوى الأول للأعمدة (Close, RSI, etc)
                data.columns = data.columns.get_level_values(0)
        
        # التأكد من تنظيف أي أعمدة مكررة أو ناتجة عن خلل في الجلب
        data = data.loc[:, ~data.columns.duplicated()]

        # التحقق من وجود عمود الإغلاق (Close)
        if 'Close' not in data.columns:
            # محاولة البحث عن العمود حتى لو كان هناك اختلاف في حالة الأحرف
            cols_map = {c.lower(): c for c in data.columns}
            if 'close' in cols_map:
                data = data.rename(columns={cols_map['close']: 'Close'})
            else:
                return None, None

        # تنظيف الصفوف الفارغة في عمود الإغلاق
        ...
        # طباعة الخطأ في الكونسول للمطور وإظهار رسالة مبسطة للمستخدم
        print(f"DEBUG: Error downloading {ticker}: {e}")
        return None, None
...
def add_indicators(df):
    ...
    # معالجة الحجم إذا كان مفقوداً أو صفراً (للمؤشرات)
    ...
    # RSI
    df['RSI'] = ta.momentum.rsi(close_series, window=14).fillna(50)
    ...
    # EMAs
    df['EMA20'] = ta.trend.ema_indicator(close_series, window=20).fillna(close_series)
    df['EMA50'] = ta.trend.ema_indicator(close_series, window=50).fillna(close_series)
    df['EMA200'] = ta.trend.ema_indicator(close_series, window=200).fillna(close_series)
    ...
    df['STOCHk'] = stoch_obj.stoch().fillna(50)
    df['STOCHd'] = stoch_obj.stoch_signal().fillna(50)
        
    # ATR (Average True Range) for Stop Loss
    df['ATR'] = ta.volatility.average_true_range(high_series, low_series, close_series, window=14).fillna(0)
    
    # Volume SMA for Breakout detection
    df['VOL_SMA'] = volume_series.rolling(window=20).mean().fillna(0)
    
    # Bollinger Bands
    bb_obj = ta.volatility.BollingerBands(close_series, window=20, window_dev=2)
    df['BB_HIGH'] = bb_obj.bollinger_hband().fillna(close_series)
    df['BB_LOW'] = bb_obj.bollinger_lband().fillna(close_series)
    
    # حساب السيولة التقريبية (Money Flow Proxy)
    df['MONEY_FLOW'] = (df['Close'] - df['Close'].shift(1)).fillna(0) * volume_series
    ...
    try:
        ...
        pass # في حال كانت البيانات قصيرة جداً
...
def calculate_wyckoff_signal(df):
    """
    تحليل وايكوف (Wyckoff) لاكتشاف التجميع والتصريف
    نستخدم VSA (Volume Spread Analysis) لتقدير نوايا صناع السوق
    """
    if df is None or len(df) < 60: return "غير كافٍ", "gray"
    ...
    avg_spread = last_30['High'].sub(last_30['Low']).mean()
    
    # حساب المتوسط المتحرك للحجم
    avg_vol = last_30['Volume'].mean()
    vol_ratio = latest['Volume'] / avg_vol if avg_vol > 0 else 1
    
    # 1. اكتشاف "الربيع" (Spring) - إشارة تجميع قوية
    local_low = last_30['Low'].min()
    is_spring = latest['Low'] <= local_low and latest['Close'] > prev['Low'] and vol_ratio > 1.2
    
    # 2. اكتشاف "الاختبار" (Testing Supply)
    is_test = spread < avg_spread and vol_ratio < 0.8 and latest['Close'] > latest['Low']
    
    # 3. اكتشاف "ذروة الشراء" (Buying Climax) - إشارة تصريف
    local_high = last_30['High'].max()
    is_climax = latest['High'] >= local_high and spread > avg_spread * 1.5 and latest['Close'] < (latest['High'] + latest['Low'])/2 and vol_ratio > 1.5

    # 4. اكتشاف "الاختراق الوهمي" (Upthrust)
    is_upthrust = latest['High'] > last_10['High'].iloc[:-1].max() and latest['Close'] < prev['Close'] and vol_ratio > 1.1

    if is_spring:
        return "💎 تجميع (Spring) - دخول محترفين", "#00ff88"
    elif is_test and latest['Close'] > df['EMA20'].iloc[-1]:
        return "🔍 اختبار نجاح (تأكيد صعود)", "#a8e6cf"
    elif is_climax or is_upthrust:
        return "🚨 تصريف (خطر خروج سيولة)", "#ff4b4b"
    elif latest['Close'] > last_30['Close'].mean() and vol_ratio > 1:
        return "📈 قوة نسبية (تحت التجميع)", "#d4edda"
    else:
        return "🔄 منطقة حياد (انتظار إشارة)", "#f0f2f6"
...
def get_ticker_data():
    ...
    for ticker in top_stocks:
        try:
            data = yf.download(ticker, period="2d", interval="1d", progress=False, timeout=10)
            if data is not None and not data.empty:
                # معالجة الأعمدة لضمان الوصول لـ Close
                if isinstance(data.columns, pd.MultiIndex):
                    if ticker in data.columns.get_level_values(0):
                        data = data[ticker]
                    else:
                ...
                # التأكد من وجود عمود Close (مع مراعاة auto_adjust)
...
@st.cache_data(ttl=60) # تحديث كل دقيقة لضمان سرعة البيانات
def get_market_summary():
    ...
    try:
        ...
        for ticker in all_tickers:
            ...
            try:
                # استخراج بيانات السهم الواحد من الدفعة بشكل آمن
                if isinstance(data_all.columns, pd.MultiIndex):
                    if ticker in data_all.columns.get_level_values(0):
                        s_data = data_all[ticker].dropna(subset=['Close'])
                    else: continue
                else:
                    s_data = data_all.dropna(subset=['Close'])
...
# تفعيل التحديث التلقائي السريع (كل 30 ثانية لضمان أقصى سرعة ممكنة)
st_autorefresh(interval=30000, key="market_summary_refresh")
...
st.sidebar.info("⏱️ التحديث التلقائي: كل 30 ثانية")
st.sidebar.warning("⚠️ تنبيه: بيانات Yahoo Finance قد تتأخر حتى 15 دقيقة عن مباشر. للمضاربة اللحظية الصفرية، يفضل استخدام اشتراك API مدفوع.")
...
if summary_data:
    ...
    # تنبيه للسيولة الضخمة (دخول سيولة) - إشعار جانبي عائم
    top_inflow = summary_data['top_inflow'].iloc[0]
    if top_inflow['money_flow'] > 0:
        inflow_msg = f"🔥 سيولة قوية تدخل سهم <b>{top_inflow['name']}</b> الآن"
        # عرض الإشعار العائم (Floating Side Notification)
        st.markdown(f"""
        ...
        """, unsafe_allow_html=True)
        # أيضاً إضافة توست سريع للتأكيد
        st.toast(f"دخول سيولة: {top_inflow['name']}", icon="💰")
...
if menu_choice == "🏠 الصفحة الرئيسية":
    ...
    if st.session_state.watchlist:
        ...
        try:
            with st.spinner("جاري تحديث بيانات أسهمي الخاصة..."):
                ...
                w_all_data = yf.download(w_tickers, period="5d", interval="1h", progress=False, group_by='ticker', timeout=15)
                ...
                for i, ticker in enumerate(w_tickers):
                    with watchlist_cols[i]:
                        try:
                            # استخراج بيانات السهم الواحد من الدفعة بشكل آمن
                            w_data = None
                            if isinstance(w_all_data.columns, pd.MultiIndex):
                                if ticker in w_all_data.columns.get_level_values(0):
                                    w_data = w_all_data[ticker].dropna(subset=['Close'])
                            else:
                                # في حال كان هناك سهم واحد فقط في القائمة، قد لا يعيد ياهو MultiIndex
                                if ticker in w_tickers:
                                    w_data = w_all_data.dropna(subset=['Close'])
                            
                            if w_data is not None and not w_data.empty and len(w_data) >= 2:
        ...
        with st.spinner(f"جاري تحليل {selected_stock_name}..."):
            try:
                ...
                if df is not None and len(df) > 10:
                    ...
                    # إشارة وايكوف
                    wyck_msg, wyck_color = calculate_wyckoff_signal(df)
                    
                    col_rec, col_wyck = st.columns([2, 1])
                    with col_rec:
                        st.markdown(f"<div class='recommendation-box {style_class}'>توصية المحلل الذكي: {rec}</div>", unsafe_allow_html=True)
                    with col_wyck:
                        st.markdown(f"""
                            <div class="modern-card" style="border-right: 5px solid {wyck_color}; text-align: center; height: 100%; display: flex; align-items: center; justify-content: center;">
                                <div>
                                    <div style="font-size: 0.8em; color: gray;">نظام وايكوف (Wyckoff)</div>
                                    <div style="font-weight: 800; color: {wyck_color if wyck_color != '#f0f2f6' else '#1e3d59'};">{wyck_msg}</div>
                                </div>
                            </div>
                        """, unsafe_allow_html=True)
                    
                    # التحليل المالي (معالجة خاصة للمؤشرات مثل تاسي)
                    st.subheader("📊 التحليل المالي الأساسي")
                    f_col1, f_col2, f_col3, f_col4 = st.columns(4)
                    
                    if ticker == "^TASI":
                        f_col1.metric("نوع الرمز", "مؤشر سوق")
                        f_col2.metric("نطاق 52 أسبوع", f"{info.get('fiftyTwoWeekLow', 'N/A')} - {info.get('fiftyTwoWeekHigh', 'N/A')}")
                        f_col3.metric("المتوسط المتحرك 50", f"{info.get('fiftyDayAverage', 'N/A')}")
                        f_col4.metric("المتوسط المتحرك 200", f"{info.get('twoHundredDayAverage', 'N/A')}")
                    else:
                        mkt_cap = info.get('marketCap', 0) / 1e9 if info.get('marketCap') else 0
                        div_yield = info.get('dividendYield', 0)
                        div_yield_pct = div_yield * 100 if div_yield else 0
                        
                        f_col1.metric("القيمة السوقية", f"{mkt_cap:.1f}B" if mkt_cap > 0 else "N/A")
                        f_col2.metric("مكرر الربحية (P/E)", f"{info.get('trailingPE', 'N/A')}")
                        f_col3.metric("ربحية السهم (EPS)", f"{info.get('trailingEps', 'N/A')}")
                        f_col4.metric("عائد التوزيعات", f"{div_yield_pct:.2f}%" if div_yield_pct > 0 else "N/A")
                    ...
                    # حجم التداول (معالجة خاصة للمؤشرات التي قد لا تحتوي على حجم)
                    ...
                    col4.metric("حجم التداول", volume_display)
                    ...
                    fig.update_layout(title=f'الرسم البياني لـ {selected_stock_name}', yaxis_title='السعر', template='plotly_white', height=500, xaxis_rangeslider_visible=False)
                    st.plotly_chart(fig, use_container_width=True)

                    # أهداف الشراء والبيع (إدارة المخاطر)
                    ...
                    with res_col2:
                        ...
                        if pd.isna(atr_val) or atr_val == 0:
                            st.info("بيانات إدارة المخاطر غير متاحة حالياً لهذا الرمز.")
                        else:
                    ...
                    st.error("⚠️ لم يتم العثور على بيانات كافية لهذا الرمز. قد يكون السوق مغلقاً أو الرمز غير صحيح.")
            except Exception as e:
                st.error(f"❌ حدث خطأ أثناء تحليل البيانات: {e}")
                st.info("حاول تحديث الصفحة أو اختيار رمز آخر.")
    ...
    with st.spinner(f"جاري مسح قطاع {target_sector}..."):
        for ticker in SECTORS[target_sector]:
            ...
            if df_radar is not None and len(df_radar) > 50:
                ...
                wyck_status, wyck_color = calculate_wyckoff_signal(df_radar)
                latest_p = df_radar['Close'].iloc[-1]
                atr_val = df_radar['ATR'].iloc[-1]
                
                # حساب الأهداف آلياً بناءً على تقلبات السهم (ATR)
                stop_loss = latest_p - (atr_val * 1.5) if not pd.isna(atr_val) else None
                target_1 = latest_p + (atr_val * 2) if not pd.isna(atr_val) else None
                ...
                radar_results.append({
                    ...
                    "قوة الفرصة": score,
                    "وايكوف (VSA)": wyck_status,
                    "وقف الخسارة": round(stop_loss, 2) if stop_loss else "N/A",
                    "الهدف 1": round(target_1, 2) if target_1 else "N/A",
                    "الإشارة": status,
    ...
    if radar_results:
        ...
        st.dataframe(
            radar_df.style.applymap(
                lambda x: 'color: #ff4b4b; font-weight: bold;' if isinstance(x, (int, float)) and x == radar_df['وقف الخسارة'].iloc[0] else '',
                subset=['وقف الخسارة']
            ).applymap(
                lambda x: 'color: #00ff88; font-weight: bold;' if isinstance(x, (int, float)) and x == radar_df['الهدف 1'].iloc[0] else '',
                subset=['الهدف 1']
            ), 
