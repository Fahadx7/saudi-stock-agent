...
def get_market_breadth():
    advancers, decliners, unchanged = 0, 0, 0
    # فحص عينة من 20 سهماً لتقدير نبض السوق
    sample = list(FLAT_STOCKS.values())[:20]
    for ticker in sample:
        if ticker == "^TASI": continue
        try:
            data = yf.download(ticker, period="2d", interval="1d", progress=False, timeout=3)
            if not data.empty:
                if isinstance(data.columns, pd.MultiIndex):
                    data.columns = data.columns.get_level_values(0)
                chg = data['Close'].iloc[-1] - data['Close'].iloc[-2]
                if chg > 0: advancers += 1
                elif chg < 0: decliners += 1
                else: unchanged += 1
        except: continue
...
def get_market_summary():
    """جلب ملخص السوق لأهم الشركات أداءً وسيولةً"""
    all_results = []
    # مسح جميع الأسهم المعرفة في القطاعات للحصول على عينة كافية
    all_tickers = list(set([stock for sublist in SECTORS.values() for stock in sublist]))
    
    for ticker in all_tickers:
        if ticker == "^TASI": continue
        try:
            data = yf.download(ticker, period="5d", interval="1d", progress=False, timeout=5)
...
# عرض نبض السوق
adv, dec, unc = get_market_breadth()
