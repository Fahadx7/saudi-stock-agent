...
STOCK_NAMES = {
    ...
    "7020.SR": "موبايلي", "7030.SR": "زین", "7203.SR": "علم", "7202.SR": "حلول", "7201.SR": "بحر العرب",
...
class DataEngine:
    ...
    def fetch_data(cls, ticker, period="1y", interval="1d"):
        try:
            stock = yf.Ticker(ticker, session=cls.get_session())
            data = stock.history(period=period, interval=interval, auto_adjust=True)
            if data.empty: data = yf.download(ticker, period=period, interval=interval, progress=False, session=cls.get_session())
        ...
        try:
            ...
            if results:
                ...
                with c1: 
                    ...
                    st.dataframe(df_res.nlargest(5, 'التغير %').style.format({'التغير %': '{:+.2f}%'}), hide_index=True, use_container_width=True)
                with c2:
                    st.subheader("📉 الأعلى هبوطاً")
                    st.dataframe(df_res.nsmallest(5, 'التغير %').style.format({'التغير %': '{:+.2f}%'}), hide_index=True, use_container_width=True)
                with c3:
                    st.subheader("🔥 الأكثر نشاطاً")
                    st.dataframe(df_res.nlargest(5, 'الحجم').style.format({'الحجم': '{:,.0f}'}), hide_index=True, use_container_width=True)
        except Exception as e:
