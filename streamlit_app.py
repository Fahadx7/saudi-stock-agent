...
def _rtl_style():
    st.markdown(
        ...
        .ticker-wrap { width: 100%; overflow: hidden; background: #0f172a; border-radius: 10px; padding: 10px 0; margin: 6px 0 14px 0; }
        .ticker-track { display: inline-block; white-space: nowrap; animation: ticker-move 35s linear infinite; color: #e2e8f0; font-weight: 700; }
        .ticker-item { display: inline-block; margin: 0 22px; }
...
def _render_top_ticker(rows):
    ...
    for r in rows:
        ...
        items.append(
            f"<span class='ticker-item'>{r['الشركة']} {r['السعر']:.2f} "
            f"<span style='color:{color}'>{chg:+.2f}%</span></span>"
        )
    if not items:
        return
    html = f"<div class='ticker-wrap'><div class='ticker-track'>{' '.join(items)}</div></div>"
    st.markdown(html, unsafe_allow_html=True)

@st.cache_data(ttl=300)
def _get_top_ticker_rows():
    tickers = [t for _, t in LIQUIDITY_UNIVERSE]
    name_map = {t: n for n, t in LIQUIDITY_UNIVERSE}
    data = _download_tickers(tuple(tickers), period="2d", interval="1d")
    if data is None or getattr(data, "empty", True):
        return []
    rows = []
    for t in tickers:
        d = _extract_ticker_frame(data, t)
        ...
        rows.append({"الشركة": name_map.get(t, t), "السعر": last, "التغير %": chg})
    return rows
...
def page_dashboard():
    ...
    st.markdown("---")
    st.subheader("💧 تدفقات السيولة (أفضل 5)")
    liquidity_data = _safe_download([t for _, t in LIQUIDITY_UNIVERSE], period="30d")
    if liquidity_data is not None and not getattr(liquidity_data, "empty", True):
        flow_rows = []
        for n, t in LIQUIDITY_UNIVERSE:
            d = _extract_ticker_frame(liquidity_data, t)
            if d is None or len(d) < 22 or "Volume" not in d.columns:
                continue
            ...
            flow_score = chg * vol_ratio
            flow_rows.append(
                {
                    "الشركة": n,
                    "الرمز": t,
                    "السعر": last_close,
                    "الحجم/متوسط20": vol_ratio,
                    "التغير %": chg,
                    "درجة السيولة": flow_score,
                }
            )
        ...
        if flow_rows:
            ...
            with in_col:
                st.markdown("**🔵 دخول سيولة**")
                st.dataframe(
                    flow_df.sort_values("درجة السيولة", ascending=False)
                    .head(5)
                    .style.format({"السعر": "{:.2f}", "الحجم/متوسط20": "{:.2f}x", "التغير %": "{:+.2f}%", "درجة السيولة": "{:.2f}"}),
                    hide_index=True,
                    width="stretch",
                )
            with out_col:
                st.markdown("**🔴 خروج سيولة**")
                st.dataframe(
                    flow_df.sort_values("درجة السيولة", ascending=True)
                    .head(5)
                    .style.format({"السعر": "{:.2f}", "الحجم/متوسط20": "{:.2f}x", "التغير %": "{:+.2f}%", "درجة السيولة": "{:.2f}"}),
                    hide_index=True,
                    width="stretch",
...
def main():
    ...
    _render_dev_panel()
    try:
        strip_rows = _get_top_ticker_rows()
        if strip_rows:
            _render_top_ticker(strip_rows)
    except Exception:
        pass
