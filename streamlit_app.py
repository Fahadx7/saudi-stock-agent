    ...
    def fetch_data(cls, ticker, period="1y", interval="1d"):
        ...
        try:
            ...
            if "Rate limited" in str(e) or "429" in str(e):
                st.warning(f"⚠️ ياهو فاينانس: تم الوصول للحد الأقصى للطلبات لرمز {ticker}. سيتم استخدام البيانات المخزنة.")
            return None, None
...
def get_market_summary_data(tickers):
    try:
        ...
        if "Rate limited" in str(e):
            st.error("⚠️ تجاوز حد الطلبات من ياهو فاينانس (Rate Limit). يرجى الانتظار قليلاً.")
        return None
...
def page_home():
    ...
    with st.spinner("جاري جلب بيانات السوق..."):
        ...
        else: st.warning("تعذر تحديث ملخص السوق")
...
def main():
    ...
    if page == "🏠 الرئيسية": st.sidebar.info("يتم تحديث الأسعار كل 5 دقائق لضمان استقرار الخدمة.")
    if page == "🏠 الرئيسية": st_autorefresh(interval=300000, key="home_refresh")
