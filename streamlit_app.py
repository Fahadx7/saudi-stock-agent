...
# ==========================================
# 🛠️ الطبقة المنطقية للبيانات (Data Layer)
# ==========================================
...
class DataEngine:
    ...
    def clean_df(df, ticker):
        ...
        # التعامل مع MultiIndex في yfinance الجديد
        ...
        # التحقق من وجود أعمدة السعر الأساسية
    ...
    @classmethod
    @st.cache_data(ttl=300) # تخزين البيانات لمدة 5 دقائق لتقليل الطلبات
    def fetch_data(cls, ticker, period="1y", interval="1d"):
        session = cls.get_session()
        try:
            # محاولة التنزيل باستخدام Ticker
            stock = yf.Ticker(ticker, session=session)
            data = stock.history(period=period, interval=interval, auto_adjust=True)
            
            # محاولة تنزيل مباشرة إذا فشلت الأولى
            if data.empty:
            ...
            return data, stock.info if data is not None else (None, {})
        except Exception as e:
            if "Rate limited" in str(e) or "429" in str(e):
                st.warning(f"⚠️ ياهو فاينانس: تجاوز حد الطلبات لـ {ticker}. سيتم الانتظار.")
            return None, {}

    @classmethod
    def get_ai_analysis(cls, stock_name, metrics):
        if "GEMINI_API_KEY" not in st.secrets: return "⚠️ مفتاح API غير متوفر في الإعدادات (Secrets)"
        try:
...
# ==========================================
# 📊 الطبقة المنطقية للتحليل (Analysis Layer)
# ==========================================
...
class AnalysisEngine:
    ...
    def get_recommendation(df):
        ...
        if last['Close'] > last['EMA20']: score += 1; reasons.append("فوق المتوسط المتحرك 20")
...
@st.cache_data(ttl=600) # تخزين ملخص السوق لمدة 10 دقائق
def get_market_summary_data(tickers):
    try:
        data = yf.download(tickers, period="2d", session=DataEngine.get_session(), progress=False, group_by='ticker', threads=False)
        return data
    except Exception as e:
        if "Rate limited" in str(e):
            st.error("⚠️ ياهو فاينانس: تجاوز حد الطلبات (Rate Limit).")
        return None

def display_ticker():
    tickers = SECTORS["القياديات"]
    data = get_market_summary_data(tickers)
    items = []
    if data is not None:
        for t in tickers:
    ...
    content = " ".join(items) if items else "جاري تحديث نبض السوق..."
...
def page_home():
    ...
    if st.session_state.get('watchlist'):
        ...
        w_data = get_market_summary_data(st.session_state.watchlist)
    ...
    # نبض السوق (Market Summary)
    all_tickers = [s for sub in SECTORS.values() for s in sub]
    with st.spinner("جاري جلب بيانات السوق السعودي..."):
        data = get_market_summary_data(all_tickers)
        if data is not None:
            results = []
            ...
            else: st.warning("السوق مغلق حالياً أو البيانات غير متوفرة.")
        else: st.warning("تعذر تحديث نبض السوق حالياً (Rate Limit).")
            ...
            with col2:
                ...
                if st.button("🤖 استشارة المحلل الذكي"):
                    with st.spinner("جاري التحليل بالذكاء الاصطناعي..."):
                        ...
                        st.info(ai_comment)
        else: st.error("عذراً، لم يتم العثور على بيانات كافية لهذا السهم.")
...
def page_radar():
    ...
    if st.button("بدء المسح اللحظي"):
        ...
        try:
            ...
            else: st.warning("لا توجد فرص واضحة حالياً في هذا القطاع.")
        except: st.error("فشل المسح بسبب ضغط الطلبات، يرجى المحاولة لاحقاً.")

def page_building():
    st.title("🏗️ مواد البناء")
    st.info("الأسعار تقديرية للسوق المحلي (تحديث يدوي)")
    c1, c2, c3 = st.columns(3)
...
def main():
    ...
    st.markdown("---")
    st.markdown('<div style="text-align:center; color:#64748b; font-size:0.8rem;">تطوير فهد AI © 2026 | جميع البيانات من Yahoo Finance</div>', unsafe_allow_html=True)
    
    if page == "🏠 الرئيسية": 
        st.sidebar.info("يتم التحديث تلقائياً كل 5 دقائق لضمان الاستقرار.")
        st_autorefresh(interval=300000, key="home_refresh")
