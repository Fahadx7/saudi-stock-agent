...
def _gemini_generate(prompt):
    ...
    if now < block_until:
        ...
        return None, f"تم إيقاف طلبات الذكاء الاصطناعي مؤقتاً بسبب الحصة. جرّب بعد {wait_s} ثانية."

    last_call = float(st.session_state.get("ai_last_call", 0.0) or 0.0)
    if now - last_call < 12:
        wait_s = int(max(1, 12 - (now - last_call)))
        return None, f"انتظر {wait_s} ثواني ثم أعد المحاولة (لتقليل استهلاك الحصة)."
        ...
        if st.button("🤖 رأي AI"):
            ...
            if err:
                ...
                if _is_quota_error(err):
                    st.info(_local_advisor_reply(name, ticker, price, rsi, opp, adx=adx, atr=atr))
            else:
                st.info(text)
